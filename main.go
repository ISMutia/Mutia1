...
func getPost(w http.ResponseWriter, r *http.Request) {
	...
	sql := `SELECT
				...
				IFNULL(Phone,'') Phone ,
				IFNULL(PostalCode,'') PostalCode
			FROM customers WHERE CustomerID = ? AND CompanyName = ? AND ContactName = ? AND ContactTitle = ?`

	result, err := db.Query(sql, CustomerID, CompanyName, ContactName, ContactTitle)
